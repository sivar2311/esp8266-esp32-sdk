...
bool AirQualitySensor<T>::sendAirQualityEvent(int pm1, int pm2_5, int pm10, String cause) {
    ...
    DynamicJsonDocument eventMessage = getDevice().prepareEvent(FSTR_AIRQUALITY_airQuality, cause.c_str());
...
template <typename T>
T& AirQualitySensor<T>::getDevice() {
    return static_cast<T&>(*this);
}

template <typename T>
const T& AirQualitySensor<T>::getDevice() const {
