...
bool TemperatureSensor<T>::sendTemperatureEvent(float temperature, float humidity, String cause) {
    ...
    DynamicJsonDocument eventMessage          = getDevice().prepareEvent(FSTR_TEMPERATURE_currentTemperature, cause.c_str());
...
template <typename T>
T& TemperatureSensor<T>::getDevice() {
    return static_cast<T&>(*this);
}

template <typename T>
const T& TemperatureSensor<T>::getDevice() const {
